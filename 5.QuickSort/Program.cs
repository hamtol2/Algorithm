...
public class Program
{
    ...
    static int Partition(int[] array, int left, int right)
    {
        ...
        // 앞서 첫번째 요소를 기준 값으로 사용했기 때문에 인덱스를 증가한 후에 비교 진행.
        ++left;

        while (left < right)
        {
            // 데이터 영역에서 피벗 값보다 큰 값을 찾을 때까지 left 인덱스 증가 처리.
            // 큰 값을 찾으면 일단 멈춤.
            // 인덱스 범위 오류까지 확인.
            ...
            // 두 인덱스가 서로 교차를 했으면, 이번 턴에서의 검색이 완료됐기 때문에 종료.
            else
            {
                break;
            }
        ...
        // 최종적으로, 피벗 값과 왼쪽 데이터 집합의 마지막 요소(right)를 교환.
        int temp2 = array[first];
        array[first] = array[right];
        array[right] = temp2;

        // 피벗 값 반환. (right를 반환하는 이유는 앞서 교환이 발생했기 때문).
        return right;
    }
